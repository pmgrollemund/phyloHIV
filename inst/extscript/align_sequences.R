...
### Run blastn ----
blastn(seq,LANL_db,max_closest,
       index,verbose)
# Run the following command for each subtype: (mixing between R and Linux command)
...
closest_result <- get_closest_seq(LANL_seq,seq,
                                  ...
                                  index,verbose)
if(is.na(index)){
 save(closest_result,file=paste(path_output,"Closest_result.rda",sep=""))
}else{
 save(closest_result,file=paste(path_output,"Closest_result_",index,".rda",sep=""))
}
...
outgroup <- add_outgroup(subtypes,
                         ...
                         index,verbose)
if(is.na(index)){
 save(outgroup,file=paste(path_output,"outgroup.rda",sep=""))
}else{
 save(outgroup,file=paste(path_output,"outgroup_",index,".rda",sep=""))
}

### Check that HXB2 is in each file  ----
names_file <- paste(path_output,output_names,".fasta",sep="")
...
indexes <- find_identical_sequences(names_file_aligned,path_sequences_meta,
                                    ...
                                    index,verbose)
if(is.na(index)){
 save(indexes,file=paste(path_output,"indexes_identical.rda",sep=""))
}else{
 save(indexes,file=paste(path_output,"indexes_identical_",index,".rda",sep=""))
}
#
